# Menus UI Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** ปรับหน้า menus ให้เป็น GL-Orbit Compact Command Hub ที่เข้ากับ project visual language และยังใช้งานไวบนมือถือ

**Architecture:** แก้เฉพาะ Svelte page component เดิม โดยคง data flow, i18n, language switching และ auth conditional rendering ไว้ทั้งหมด UI จะใช้ design tokens/global utilities ที่มีอยู่ เช่น `glass-card`, `bg-gradient-mesh`, `touch-target`, coral/lavender/mint/plum colors

**Tech Stack:** SvelteKit 2, Svelte 5 runes, TypeScript, Tailwind CSS 4, Paraglide i18n

## Global Constraints

- ปรับ UI เฉพาะหน้า `src/routes/[lang=lang]/(app)/menus/+page.svelte`
- ใช้ i18n keys เดิม ไม่เพิ่มข้อความใหม่ถ้าไม่จำเป็น
- คง behavior เดิม: login user เห็น profile card, guest เห็น login/register cards, language switching ทำงานเหมือนเดิม
- แก้ syntax artifact `003e` ถ้ายังอยู่
...
- ระวัง mobile overflow จาก decorative backgrounds
- Verification หลักคือ `npm run check`
...
- Modify: `src/routes/[lang=lang]/(app)/menus/+page.svelte`
  - Responsibility: render compact menu hub UI, account action card(s), language selector, SEO metadata, and language switching behavior.
- No new components: scopeเล็กและ UI ใช้เฉพาะหน้านี้ การแยก component เพิ่มจะเกินจำเป็น
- No message file changes: use existing `m.menus_*`, `m.nav_*`, and language labels

---

### Task 1: Replace menus page markup with compact Orbit Hub UI
...
**Interfaces:**
- Consumes:
  - `currentUser` derived value from `page.data.user`
  - `currentLang` derived value from route/lang data
  - `switchLanguage(lang: AvailableLanguageTag): Promise<void>` existing function
  - `localizedHref(path: string, lang: AvailableLanguageTag | string)` existing helper
  - existing `m.*` i18n functions
- Produces:
  - Same route UI with compact Orbit Hub visual treatment
  - Same links to `/profile`, `/login`, `/register`
  - Same language buttons with `aria-pressed`

- [ ] **Step 1: Capture current status before editing**
...
Expected: shows existing modified `src/routes/[lang=lang]/(app)/menus/+page.svelte` plus committed spec history may already be clean for docs.

- [ ] **Step 2: Replace the Svelte markup after `</svelte:head>`**

In `src/routes/[lang=lang]/(app)/menus/+page.svelte`, keep the entire `<script lang="ts">...</script>` block and `<svelte:head>...</svelte:head>` block unchanged. Replace everything after `</svelte:head>` with this markup:
...
- [ ] **Step 3: Verify the syntax artifact is gone**

Run:

```bash
rg -n "003e|<span class=\"font-\[family-name:var\(--font-display\)\] text-xl font-black text-coral-dark\"$" 'src/routes/[lang=lang]/(app)/menus/+page.svelte'
```
...
Expected: command exits with code 0. If it fails because of unrelated existing errors, capture the exact first relevant error before changing anything else.
...
```bash
git diff -- 'src/routes/[lang=lang]/(app)/menus/+page.svelte'
```

Expected: diff only changes markup after `</svelte:head>` in `menus/+page.svelte`; script behavior and SEO head are unchanged.
...
```bash
git add 'src/routes/[lang=lang]/(app)/menus/+page.svelte'
git commit -m "style: polish compact menus hub"
```
...
- Spec coverage: The single task covers compact Orbit Hub UI, existing behavior preservation, language active state, syntax artifact removal, mobile overflow caution, and `npm run check` verification.
- Placeholder scan: No TBD/TODO/implement-later placeholders remain.
- Type consistency: No new TypeScript interfaces or functions are introduced; all referenced values already exist in the page script.
