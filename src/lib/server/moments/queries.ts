...
import { momentArtists, momentBookmarks, momentLikes, momentMedia, momentSeries, momentShips, moments, users } from '../db/schema.js';
...
export async function getMoments(filter: MomentFilter = {}) {
	...
	const [media, seriesTags, artistTags, shipTags, likedRows, bookmarkedRows] = ids.length ? await Promise.all([db.select({ id: momentMedia.id, momentId: momentMedia.momentId, externalUrl: momentMedia.externalUrl, altText: momentMedia.altText, sortOrder: momentMedia.sortOrder }).from(momentMedia).where(sql`${momentMedia.momentId} = any(${ids})`).orderBy(momentMedia.sortOrder), db.select().from(momentSeries).where(sql`${momentSeries.momentId} = any(${ids})`), db.select().from(momentArtists).where(sql`${momentArtists.momentId} = any(${ids})`), db.select().from(momentShips).where(sql`${momentShips.momentId} = any(${ids})`), filter.viewerId ? db.select({ momentId: momentLikes.momentId }).from(momentLikes).where(and(eq(momentLikes.userId, filter.viewerId), inArray(momentLikes.momentId, ids))) : Promise.resolve([]), filter.viewerId ? db.select({ momentId: momentBookmarks.momentId }).from(momentBookmarks).where(and(eq(momentBookmarks.userId, filter.viewerId), inArray(momentBookmarks.momentId, ids))) : Promise.resolve([])]) : [[], [], [], [], [], []];
	const likedIds = new Set(likedRows.map(({ momentId }) => momentId));
	const bookmarkedIds = new Set(bookmarkedRows.map(({ momentId }) => momentId));
	return { moments: page.map((row) => ({ ...row, liked: likedIds.has(row.id), bookmarked: bookmarkedIds.has(row.id), author: serializeMomentAuthor(row), media: media.filter((item) => item.momentId === row.id), seriesIds: seriesTags.filter((item) => item.momentId === row.id).map((item) => item.seriesId), artistIds: artistTags.filter((item) => item.momentId === row.id).map((item) => item.artistId), shipIds: shipTags.filter((item) => item.momentId === row.id).map((item) => item.shipId) })), nextCursor: rows.length > limit ? encodeCursor({ createdAt: page.at(-1)!.createdAt.toISOString(), id: page.at(-1)!.id }) : null };
}
