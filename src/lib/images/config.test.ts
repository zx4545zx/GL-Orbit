...
describe('deriveVariantUrls', () => {
	...
	it('derives only new cover variants from a 1440 canonical URL', () => {
		const v = deriveVariantUrls(`${BASE}/covers/${UUID}/1440.jpg`, 'covers');
		expect(v!.avif.map((e) => e.width)).toEqual([960, 1440]);
		expect(v!.jpg.at(-1)?.url).toBe(`${BASE}/covers/${UUID}/1440.jpg`);
	});

	it('preserves all legacy cover variants from an 1800 canonical URL', () => {
...
describe('IMAGE_VARIANTS', () => {
	it('has expected widths and fallback', () => {
		...
		expect(IMAGE_VARIANTS.covers.widths).toEqual([960, 1440]);
		expect(IMAGE_VARIANTS.covers.fallback).toBe(1440);
		expect(LEGACY_COVER_WIDTHS).toEqual([960, 1440, 1800]);
