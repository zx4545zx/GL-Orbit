...
<div class="space-y-5">
	...
	<div class="flex flex-col sm:flex-row gap-5">
		<div class="sm:w-40 flex-shrink-0">
			<span class="block text-sm font-medium text-plum mb-1.5">โปสเตอร์</span>
			<div class="relative w-32 sm:w-40 aspect-[2/3] rounded-2xl overflow-hidden bg-lavender/10 border border-lavender/20">
				{#if posterUrl}
				...
				{:else}
					<div class="w-full h-full flex flex-col items-center justify-center text-lavender-dark/50 gap-1 p-2">
						...
						<span class="text-[10px] text-center">ไม่มีโปสเตอร์</span>
					</div>
				{/if}
			</div>
			<input
				...
				placeholder="วาง URL รูปโปสเตอร์"
				class="mt-2 w-full px-3 py-2 rounded-xl border border-lavender/30 bg-white/60 text-plum focus:outline-none focus:ring-2 focus:ring-coral/30 text-xs"
			/>
		</div>
