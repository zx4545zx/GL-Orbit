...
<div class="space-y-6">
	...
	<section id="series-details" role="tabpanel" class="overflow-hidden rounded-3xl border border-lavender/20 bg-white/55 shadow-lg shadow-lavender/5">
		<div class="border-b border-lavender/15 bg-coral/5 px-4 py-4 sm:px-6">
		...
		<div class="space-y-6 p-4 sm:p-6">
			...
			<div class="border-t border-lavender/15 pt-6"><div class="mb-4"><h3 class="text-base font-bold text-plum">ภาพปกซีรีส์</h3><p class="text-sm text-plum-light">โปสเตอร์และภาพปกจะบันทึกพร้อมข้อมูลหลัก</p></div><div class="grid gap-4 sm:grid-cols-2"><div class="rounded-2xl border border-lavender/15 bg-white/60 p-3"><ImageUpload bind:url={posterUrl} type="posters" purpose="poster" label="โปสเตอร์" /></div><div class="rounded-2xl border border-lavender/15 bg-white/60 p-3"><ImageUpload bind:url={coverUrl} type="covers" purpose="cover" label="ภาพปกซีรีส์" /></div></div></div>
			{#if error}<p class="rounded-xl bg-coral/10 px-3 py-2 text-sm text-coral-dark">{error}</p>{/if}
			<div class="flex flex-wrap items-center justify-end gap-3 border-t border-lavender/15 pt-5">{#if metadataDirty}<span class="text-sm text-plum-light">มีการแก้ไขที่ยังไม่บันทึก</span>{/if}{#if saved}<span class="text-sm font-medium text-mint-dark">บันทึกแล้ว</span>{/if}<button type="button" onclick={save} disabled={saving || !metadataDirty} class="orbit-action min-h-11 rounded-xl px-5 text-sm font-bold transition disabled:cursor-not-allowed disabled:opacity-50">{saving ? 'กำลังบันทึก...' : 'บันทึกข้อมูลหลัก'}</button></div>
		</div>
	</section>
	...
	<section id="series-gallery" role="tabpanel" class="overflow-hidden rounded-3xl border border-lavender/20 bg-white/55 shadow-lg shadow-lavender/5">
		<div class="border-b border-lavender/15 bg-lavender/10 px-4 py-4 sm:px-6"><p class="text-xs font-bold uppercase tracking-[0.2em] text-lavender-dark">Gallery</p><h2 class="mt-1 text-lg font-bold text-plum">Gallery รูปซีรีส์</h2><p class="mt-1 text-sm text-plum-light">เพิ่ม ลบ และจัดลำดับรูปจะบันทึกทันที</p></div>
		<div class="space-y-6 p-4 sm:p-6">
