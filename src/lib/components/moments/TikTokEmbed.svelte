...
<div class="max-w-[420px] overflow-hidden rounded-2xl border border-[#ded8df] bg-black">
	...
	{:else}
		<button type="button" onclick={() => { warmup(); activated = true; }} onmouseenter={warmup} onfocus={warmup} ontouchstart={warmup} class="halo-focus-ring group relative block aspect-[9/16] w-full bg-black" aria-label={`เล่น ${title}`}>
			{#if poster}<img src={poster} alt="" loading="lazy" decoding="async" class="h-full w-full object-contain" />{:else}<span class="grid h-full place-items-center bg-plum text-white"><HaloIcon name="music" size={48} /></span>{/if}
			<span class="absolute left-1/2 top-1/2 grid h-16 w-16 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full bg-black/70 text-white transition group-hover:scale-110"><HaloIcon name="play" size={28} /></span>
		</button>
	{/if}
	<a href={postUrl} target="_blank" rel="noreferrer" class="halo-focus-ring flex items-center justify-between border-t border-white/10 bg-white px-4 py-3 text-xs"><span class="font-medium text-plum">เปิดดูที่ต้นฉบับ</span><span class="inline-flex items-center gap-1 text-coral-dark">TikTok <HaloIcon name="external" size={14} /></span></a>
</div>
