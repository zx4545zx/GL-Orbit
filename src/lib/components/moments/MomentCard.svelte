		...
		<div class="min-w-0 flex-1">
			...
			{#if images.length}
				<div class="relative mt-3 aspect-[4/5] max-h-[560px] overflow-hidden rounded-2xl bg-plum/[.04]" role="region" aria-roledescription="carousel" aria-label={isThai ? `รูปภาพโพสต์ ${images.length} รูป` : `${images.length} post images`}>
					...
					{/each}
					{#if images.length > 1}
						<button type="button" onclick={showPreviousImage} class="halo-focus-ring absolute left-2 top-1/2 grid h-9 w-9 -translate-y-1/2 place-items-center rounded-full bg-white/90 text-lg text-plum shadow-sm transition hover:bg-white" aria-label={isThai ? 'รูปก่อนหน้า' : 'Previous image'}><HaloIcon name="chevron-left" size={18} /></button>
						<button type="button" onclick={showNextImage} class="halo-focus-ring absolute right-2 top-1/2 grid h-9 w-9 -translate-y-1/2 place-items-center rounded-full bg-white/90 text-lg text-plum shadow-sm transition hover:bg-white" aria-label={isThai ? 'รูปถัดไป' : 'Next image'}><HaloIcon name="chevron-right" size={18} /></button>
...
{#if fullImageOpen && images[mediaIndex]}
	<div class="fixed inset-0 z-50 grid place-items-center bg-plum/90 p-4" role="dialog" aria-modal="true" aria-label={isThai ? 'ภาพขนาดเต็ม' : 'Full-size image'}>
		<button type="button" onclick={() => fullImageOpen = false} class="halo-focus-ring absolute right-4 top-4 z-10 grid h-11 w-11 place-items-center rounded-full bg-white/15 text-2xl text-white transition hover:bg-white/25" aria-label={isThai ? 'ปิดภาพขนาดเต็ม' : 'Close full-size image'}><HaloIcon name="x" size={20} /></button>
		<div class="relative h-full w-full" role="region" aria-roledescription="carousel" aria-label={isThai ? `รูปภาพโพสต์ ${images.length} รูป` : `${images.length} post images`}>
			...
			{#if images.length > 1}
				<button type="button" onclick={showPreviousImage} class="halo-focus-ring absolute left-0 top-1/2 grid h-11 w-11 -translate-y-1/2 place-items-center rounded-full bg-white/15 text-2xl text-white transition hover:bg-white/25" aria-label={isThai ? 'รูปก่อนหน้า' : 'Previous image'}><HaloIcon name="chevron-left" size={22} /></button>
				<button type="button" onclick={showNextImage} class="halo-focus-ring absolute right-0 top-1/2 grid h-11 w-11 -translate-y-1/2 place-items-center rounded-full bg-white/15 text-2xl text-white transition hover:bg-white/25" aria-label={isThai ? 'รูปถัดไป' : 'Next image'}><HaloIcon name="chevron-right" size={22} /></button>
