...
{#snippet seriesCard(s: FavoriteSeriesItem | WatchedSeriesItem)}
	<a href="/series/{s.id}" class="group block">
				...
				{:else}
					...
					<div in:fly={{ y: 8, duration: 250 }} class="lg:grid lg:grid-cols-[minmax(0,13rem)_minmax(0,1fr)] xl:grid-cols-[minmax(0,15rem)_minmax(0,1fr)] lg:gap-6">
						...
						<aside class="lg:sticky lg:top-24 lg:self-start mb-4 lg:mb-0 space-y-3">
							...
							<!-- account actions (desktop only — mobile shows them at the bottom) -->
							<div class="hidden lg:block space-y-2.5">
								{#if profileUser.role === 'ADMIN'}
									<a href="/admin/series" class="w-full py-2.5 px-4 rounded-xl bg-gradient-to-r from-coral to-coral-dark text-white font-semibold shadow-lg shadow-coral/25 hover:shadow-xl hover:scale-[1.02] transition-all duration-300 text-sm touch-target flex items-center justify-center gap-2">
						...
						<!-- account actions (mobile only — desktop has them in the sidebar) -->
						<div class="lg:hidden mt-4 pt-4 border-t border-lavender/15 space-y-2.5">
							{#if profileUser.role === 'ADMIN'}
								<a href="/admin/series" class="w-full py-3 px-4 rounded-xl bg-gradient-to-r from-coral to-coral-dark text-white font-semibold shadow-lg shadow-coral/25 hover:shadow-xl hover:scale-[1.02] transition-all duration-300 text-sm sm:text-base touch-target flex items-center justify-center gap-2">
