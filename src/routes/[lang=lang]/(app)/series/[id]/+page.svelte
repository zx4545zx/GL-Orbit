												...
												{#each item.schedules as schedule}
													...
													<div class="flex min-w-0 max-w-full items-center justify-between gap-2 rounded-xl bg-cream/70 px-3 py-2.5 min-[360px]:gap-3">
														<div class="flex min-w-0 flex-1 items-center gap-2 min-[360px]:gap-3">
															...
															<div class="min-w-0 flex-1"><p class="truncate text-sm font-bold text-plum">{schedule.title || schedule.platform}</p>{#if schedule.title}<p class="truncate mt-0.5 text-[10px] font-semibold text-plum-light/55">{schedule.platform}</p>{/if}<p class="truncate mt-0.5 text-[10px] font-semibold text-plum-light/55">{schedule.airDate}{schedule.isUncut ? ` · ${m.common_uncut()}` : ''}</p></div>
														</div>
