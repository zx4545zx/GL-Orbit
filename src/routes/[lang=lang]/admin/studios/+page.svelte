...
<div class="py-6 sm:py-8">
	<div class="flex flex-col sm:flex-row sm:items-center justify-between mb-5 sm:mb-6 gap-4">
		...
		<button onclick={() => (showCreate = !showCreate)} class="px-5 sm:px-6 py-2.5 sm:py-3 rounded-xl bg-gradient-to-r from-coral to-coral-dark text-white font-semibold shadow-lg shadow-coral/25 hover:shadow-xl hover:scale-105 transition-all flex items-center justify-center gap-2 text-sm sm:text-base touch-target">
	...
	<form class="mb-4 sm:mb-6" onsubmit={(e) => e.preventDefault()}>
		<div class="relative">
			...
			<input type="text" bind:value={search} placeholder="ค้นหาชื่อสตูดิโอ..." class="w-full pl-11 pr-4 py-2.5 rounded-xl border border-lavender/30 bg-white/70 text-plum focus:outline-none focus:ring-2 focus:ring-coral/30 text-sm" />
		</div>
	</form>
		...
		<div class="glass-card rounded-2xl sm:rounded-3xl p-4 sm:p-6 mb-6 shadow-lg shadow-lavender/5">
			...
			<form onsubmit={createStudio} class="space-y-4">
				...
				<div class="flex gap-2 pt-1">
					<button type="submit" disabled={createLoading} class="px-5 py-2.5 rounded-xl bg-gradient-to-r from-coral to-coral-dark text-white font-semibold shadow-lg shadow-coral/25 hover:shadow-xl transition-all text-sm sm:text-base touch-target disabled:opacity-50">{createLoading ? 'กำลังสร้าง...' : 'สร้าง & จัดการ'}</button>
					<button type="button" onclick={() => (showCreate = false)} class="px-5 py-2.5 rounded-xl bg-gray-100 text-gray-700 font-medium hover:bg-gray-200 transition-all text-sm sm:text-base touch-target">ยกเลิก</button>
				</div>
			</form>
		</div>
	{/if}
