...
<div class="py-4 sm:py-6">
	<div class="flex items-center gap-3 mb-5">
		<button onclick={() => goto('/admin/artists')} class="p-2 -ml-2 rounded-xl hover:bg-lavender/15 text-plum transition-colors touch-target flex-shrink-0" aria-label="กลับ">
					...
					<div class="space-y-5">
						...
						<div class="flex flex-col sm:flex-row gap-5">
							<div class="sm:w-36 flex-shrink-0">
								...
								<div class="w-28 h-28 sm:w-36 sm:h-36 rounded-full overflow-hidden bg-lavender/10 border border-lavender/20 flex items-center justify-center">
									{#if profileImageUrl}<img src={profileImageUrl} alt={nickname} class="w-full h-full object-cover" />{:else}<svg class="w-12 h-12 text-lavender-dark/50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>{/if}
								</div>
							</div>
							<div class="flex-1 space-y-4">
								...
								<div>
									...
									<input id="artist-profile-image" type="url" bind:value={profileImageUrl} class="w-full px-4 py-2.5 rounded-xl border border-lavender/30 bg-white/60 text-plum focus:outline-none focus:ring-2 focus:ring-coral/30 text-sm sm:text-base" />
								</div>
							</div>
						</div>
