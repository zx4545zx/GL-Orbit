...
import { deleteMoment, InvalidMomentSeriesError, updateMoment } from '$lib/server/moments/mutations.js';
...
export const PATCH: RequestHandler = async ({ locals, params, request }) => {
	...
	const input = await parseMomentRequest(request);
	if (!input || !input.stagedMedia.every((media) => isStagedMomentMedia(media, params.id, user.id))) return json({ error: 'INVALID_MOMENT' }, { status: 400 });
	try {
		return await updateMoment(params.id, user.id, input) ? json({ success: true }) : json({ error: 'NOT_FOUND' }, { status: 404 });
	} catch (error) {
		return json({ error: error instanceof InvalidMomentSeriesError ? 'INVALID_MOMENT' : 'UPDATE_FAILED' }, { status: error instanceof InvalidMomentSeriesError ? 400 : 500 });
	}
};
